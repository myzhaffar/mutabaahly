...
import { BookOpen, Award, ChevronLeft, Filter } from 'lucide-react';
...
const Students = () => {
  ...
  const hasAnyActiveFilter = (selectedGrades.length > 0 && !selectedGrades.includes('all')) || 
                            (selectedClasses.length > 0 && !selectedClasses.includes('all'));
  ...
  const handleClassToggle = (classId: string) => {
    setSelectedClasses((prev) => {
      ...
      if (classId === 'all') {
        ...
        updated = prev.includes(classId)
          ? prev.filter(c => c !== classId && c !== 'all')
          : [...prev.filter(c => c !== 'all'), classId];
      }
      return updated.length === 0 ? ['all'] : updated;
    });
  };
                ...
                <div className="flex items-center gap-2 flex-wrap mt-2">
                  {selectedGrades.length > 0 && !selectedGrades.includes('all') && (
                    <span className="inline-flex items-center gap-1 px-2 py-1 sm:px-3 sm:py-1 bg-green-50 text-green-700 text-xs sm:text-sm rounded-full border border-green-200">
                      Grades: {selectedGrades.map(id => gradeOptions.find(g => g.id === id)?.name || id).join(', ')}
                    </span>
                  )}
                  {selectedClasses.length > 0 && !selectedClasses.includes('all') && (
                    <span className="inline-flex items-center gap-1 px-2 py-1 sm:px-3 sm:py-1 bg-blue-50 text-blue-700 text-xs sm:text-sm rounded-full border border-blue-200">
                      Classes: {selectedClasses.map(id => classOptions.find(c => c.id === id)?.name || id).join(', ')}
                    </span>
                  )}
                  <button
                    type="button"
          ...
          {filtersOpen && (
              <div className="p-4 sm:p-6">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6">
                  ...
                  {/* Class Multi-Select Filter (Checkboxes) */}
                  <div className="space-y-2 sm:space-y-3">
                    <label className="text-xs sm:text-sm font-medium text-gray-700 flex items-center gap-2">
                      ...
                      {classOptions.map((classOption) => (
                        <div key={classOption.id} className="flex items-center space-x-2">
                          <Checkbox
                            ...
                            onCheckedChange={() => handleClassToggle(classOption.id)}
                            className="rounded-full"
                          />
                          <label htmlFor={`class-${classOption.id}`} className="text-sm text-gray-600 cursor-pointer">
                            {classOption.name}
                          </label>
                        </div>
                      ))}
