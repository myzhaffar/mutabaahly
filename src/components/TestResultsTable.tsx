...
import { TilawatiTest } from "@/types/tilawati"; // Import shared type

interface TestResultsTableProps {
  tests: TilawatiTest[]; // Use shared type
  onEditTest: (test: TilawatiTest) => void;
  onDeleteTest: (testId: string) => void;
  onViewTestDetails?: (test: TilawatiTest) => void;
}

// Helper function for badge variants (ensure your Badge component supports these or adjust class names)
const getStatusBadgeVariant = (status: TilawatiTest['status']) => {
  switch (status) {
    case 'passed': return 'success'; // You might need to define these variants in your ui/badge.tsx
    case 'failed': return 'destructive';
    case 'scheduled': return 'default'; // Or 'info' or similar
    case 'pending_retake': return 'warning';
    case 'cancelled': return 'outline'; // Or 'secondary'
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-gray-50 hover:bg-gray-100/50 border-b border-gray-200">
            ...
            <TableHead className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase tracking-wider">Jilid Skrg.</TableHead>
            <TableHead className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase tracking-wider">Target Jilid</TableHead>
            <TableHead className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase tracking-wider">Tgl Tes</TableHead>
            <TableHead className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase tracking-wider">Penguji</TableHead>
            <TableHead className="px-4 py-3 text-xs font-semibold text-gray-700 uppercase tracking-wider">Status</TableHead>
            <TableHead className="text-center px-4 py-3 text-xs font-semibold text-gray-700 uppercase tracking-wider">Nilai</TableHead>
          ...
          {tests.map((test) => (
            <TableRow key={test.id!} className="hover:bg-gray-50/50 transition-colors duration-150">
              <TableCell className="font-medium px-4 py-3 text-sm text-gray-800 whitespace-nowrap">
                {test.students?.name || test.student_id} 
              </TableCell>
              <TableCell className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{test.current_jilid}</TableCell>
              <TableCell className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{test.target_jilid}</TableCell>
              <TableCell className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{formatDate(test.test_date)}</TableCell>
              <TableCell className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{test.examiner_name || '-'}</TableCell>
              <TableCell className="px-4 py-3 text-sm whitespace-nowrap">
                <Badge variant={getStatusBadgeVariant(test.status) as any} className="capitalize text-xs font-medium px-2 py-0.5 rounded-full">
                  {test.status.replace('_', ' ')}
                </Badge>
              </TableCell>
              <TableCell className="text-center px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{test.score ?? '-'}</TableCell>
